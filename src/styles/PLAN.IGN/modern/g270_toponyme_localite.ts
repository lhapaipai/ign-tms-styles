...
export const g270_toponyme_localite: LayerSpecification[] = [
  ...
  },
  {
     ...
     */
    filter: ["match", ["get", "txt_typo"], ["commune 3", "TYPO_A_3", "BAT_COMMUNE_3", "BAT_COMMUNE_3_T"], true, false],
    layout: {
      "icon-image": "Localite",
      "icon-size": 0.21,
      "text-field": ["to-string", ["get", "texte"]],
...
// console.log(`nouveau ${g270_toponyme_localite.length}`);
// console.log(g270_toponyme_localite.map((s) => `${s.id}: ${s.minzoom} -> ${s.maxzoom} : ${s.filter.toString()}`).join("\n"));
